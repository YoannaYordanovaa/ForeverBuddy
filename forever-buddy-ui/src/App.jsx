    ...
    try {
      ...
      if (
        ...
        const response = await fetch(
          "https://foreverbuddy.onrender.com/learn",
          {
            method: "POST",
        ...
        const response = await fetch(
          "https://foreverbuddy.onrender.com/forget",
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ text: fact }),
      ...
      setMessages((prev) => [
        ...prev,
        { sender: "bot", text: "Грешка при връзката със сървъра." },
      ]);
    } finally {
      setIsLoading(false);
    }
        ...
        {messages.map((msg, index) => (
          <div key={index} className={`message-row ${msg.sender}`}>
            <div className={`bubble ${msg.sender}`}>
              {/* НОВО: Името на бота над самото съобщение */}
              {msg.sender === "bot" && (
                <div className="bot-name">
                  <img src="/Icon.svg" alt="Avatar" className="bot-avatar" />{" "}
                  <strong style={{ color: "#6A0DAD" }}>Forever</strong>
                  <em style={{ color: "#FFC300", fontWeight: "bold" }}>
                    Buddy
                  </em>
                </div>
              )}
              {/* 1. Началното съобщение с цветно лого */}
              {index === 0 ? (
                <span>
              ...
              ) : /* 2. Индикаторът за мислене (показва се само ако е бот и текстът е празен) */
              msg.sender === "bot" && msg.text === "" ? (
                ...
                /* 3. Нормалните съобщения (форматирани с Markdown) */
